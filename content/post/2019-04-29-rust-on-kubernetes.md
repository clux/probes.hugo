...
The details of `Cache<T>` is a little more involved, but the state you ultimately get out of it is a `BTreeMap<String, T>`, where the key is `Named` how you like (probably using the `.name` key of the CRD).
...
create your state, containing `Reflector<FooResource>`:
...
This is gives you a shareable `Reflector`, but you also need to drive it. Let's make a nice constructor with some methods on it, since it'll be the interface you'll use from your application (and maybe you'll need more resources):
